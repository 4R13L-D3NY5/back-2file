...
class AsignaturaController extends Controller
{
    ...
    /**
     * Mostrar documentación completa de una asignatura en formato JSON (visualización directa)
     * GET /api/asignaturas/{id}/json
     * Acceso: Solo DIRECTOR_CARRERA y SUPER_ADMIN (controlado por middleware)
     */
    public function showJson(Request $request, $id)
    {
        Log::info('AsignaturaController::showJson invoked', [
            'id' => $id,
            'user_id' => $request->user()?->id,
            'user_role' => $request->user()?->rol?->codigo,
        ]);
        
        // Reutilizar la lógica de exportJson pero sin descarga
        $asignatura = Asignatura::query();
        ...
        // Devolver JSON para visualización en navegador (sin descarga)
        return response()->json($jsonData, 200, [], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
    }

    /**
     * Mostrar documentación completa de una asignatura en formato JSON por código (visualización directa)
     * GET /api/asignaturas/codigo/{codigo}/json
     * Acceso: Solo DIRECTOR_CARRERA y SUPER_ADMIN (controlado por middleware)
     */
    public function showJsonByCode(Request $request, $codigo)
    {
        Log::info('AsignaturaController::showJsonByCode invoked', [
            'codigo' => $codigo,
            'user_id' => $request->user()?->id,
            'user_role' => $request->user()?->rol?->codigo,
        ]);
        
        // Reutilizar la lógica de exportJsonByCode pero sin descarga
        $query = Asignatura::query();
